...
const graphicElements = computed(() =>
  data.filter((item) => item.type === 'symbol')
);
const texts = computed(() => data.filter((item) => item.type === 'text'));
...
const computeLogoLayout = async () => {
  ...
  const imageBounding = image.value[0].getBoundingClientRect();
  const textBounding = logoText.value[0].getBoundingClientRect();

  const logoWidth = Math.max(imageBounding.width, textBounding.width);
  const logoHeight = imageBounding.height + textBounding.height;
  console.log(imageBounding, textBounding);
...
<template>
  <svg class="root-svg">
    <svg
      ref="image"
      v-for="graphic in graphicElements"
      :key="graphic.id"
